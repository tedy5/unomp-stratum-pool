...
var location = 'EU';
function callbackMAIL(error, response, body) {
...
var pool = module.exports = function pool(options, authorizeFn, logger) {
    ...
    function SubmitBlock(blockHex, callback) {
        var optionsMAIL = {
        url: 'http://change.me/notification/node.php?coin=' + options.coin.name + '&algo=' options.coin.algorithm + '&location=' + location
        };
            ...
            function (results) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    if (result.error) {
                        ...
                        request(optionsMAIL, callbackMAIL);
                        return;
                    }
                }
                emitLog('Submitted Block using ' + rpcCommand + ' successfully to daemon instance(s)');
                request(optionsMAIL, callbackMAIL); //remove this one later if works
                callback();
            }
        );
