...
//                logger.debug('error = %s', error);
    ...
    function SetupPeer() {
        ...
        _this.peer.on('connected', function () {
            ...
            emitWarningLog('p2p had a socket error, err = %s', JSON.stringify(e));
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            finishedCallback();
                ...
                emitLog('[GREPME] No new blocks for ' + options.jobRebroadcastTimeout + ' seconds - updating transactions & rebroadcasting work');
...
//          if (typeof options.blockRefreshInterval !== "number" || options.blockRefreshInterval <= 0) {
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(function (error, result, foundNewBlock) {
                if (foundNewBlock) {
                    emitLog('[GREPME] Block notification via RPC polling');
                }
            });
        }, pollingInterval);
    }
    ...
    this.processBlockNotify = function (blockHash, sourceTrigger) {
        emitInfoLog('[GREPME] Block notification via ' + sourceTrigger);
