...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    var emitLog        = function(text) { _this.emit('logDebug', 'debug'  , text); };
    var emitInfoLog    = function(text) { _this.emit('logInfo', 'info'   , text); };
    var emitWarnLog    = function(text) { _this.emit('logWarn', 'warn'   , text); };
    var emitErrorLog   = function(text) { _this.emit('logError', 'error'  , text); };
...
//                logger.debug('error = %s', error);
         ...
         var generateProgress = function() {
            ...
            _this.daemon.cmd(cmd, [], function(results) {
                ...
                _this.daemon.cmd('getpeerinfo', [], function(results) {
                    ...
                    emitWarnLog('Downloaded ' + percent + '% of blockchain from ' + peers.length + ' peers');
                });
    ...
    function SetupPeer() {
        ...
        _this.peer.on('connected', function () {
            ...
            emitWarnLog('p2p peer node disconnected - attempting reconnection...');
        }).on('connectionFailed', function (e) {
            emitWarnLog('p2p connection failed - likely incorrect host or port');
        }).on('socketError', function (e) {
            emitWarnLog('p2p had a socket error, err = %s', JSON.stringify(e));
        }).on('error', function (msg) {
            emitWarnLog('p2p had an error ' + msg);
        ...
        _this.daemon.cmd(rpcCommand,
            ...
            function (results) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    emitInfoLog("Response from submitblock: " + JSON.stringify(result));
                    if (result.error) {
                ...
                emitInfoLog('Submitted Block using ' + rpcCommand + ' successfully to daemon instance(s)');
                callback();
            }
        );
...
//            emitWarnLog('No rewardRecipients have been setup which means no fees will be taken');
//        }
...
//                logger.silly('mining.submit event, params = %s', JSON.stringify(params));
                ...
                emitWarnLog('Malformed message from ' + client.getLabel() + ': ' + message);

            }).on('socketError', function(err) {
                emitWarnLog('Socket error from ' + client.getLabel() + ': ' + JSON.stringify(err));

            }).on('socketTimeout', function(reason){
                emitWarnLog('Connected timed out for ' + client.getLabel() + ': ' + reason)
                ...
                emitWarnLog('Detected socket flooding from ' + client.getLabel());
                ...
                emitWarnLog('Booted worker ' + client.getLabel() + ' who was connected from an IP address that was just banned');

            }).on('triggerBan', function(reason){
                emitWarnLog('Banned triggered for ' + client.getLabel() + ': ' + reason);
    ...
    function SetupBlockPolling() {
        if (typeof options.blockRefreshInterval !== "number" || options.blockRefreshInterval <= 0 || options.coin.disablePolling == "true") {
//          if (typeof options.blockRefreshInterval !== "number" || options.blockRefreshInterval <= 0) {
//            emitLog('Block template polling has been disabled');
    ...
    function CheckBlockAccepted(blockHash, blockHex, daemon, callback) {
        //setTimeout(function(){
console.log('BLOCKHAAAAAAAAAAAAAAAAAASH: ' + blockHash);
