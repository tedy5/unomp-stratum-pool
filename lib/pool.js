...
var pool = module.exports = function pool(options, authorizeFn, logger, poolConfig) {
    ...
    if (!(options.coin.algorithm in algos)) {
        emitErrorLog('The %s hashing algorithm is not supported.', options.coin.algorithm);
        throw new Error();
    }
    ...
    function OnBlockchainSynced(syncedCallback) {
        ...
        checkSynced(function () {
            //Only let the first fork show synced status or the log wil look flooded with it
            if (!process.env.forkId || process.env.forkId === '0')
                emitWarningLog('Daemon is still syncing with network (download blockchain) - server will be started once synced');
        });
    ...
    function SetupPeer() {
        ...
        _this.peer.on('connected', function () {
            emitInfoLog('p2p connection successful');
            ...
            emitWarningLog('p2p connection failed - likely incorrect host or port');
        }).on('socketError', function (e) {
            emitErrorLog('p2p had a socket error, err = %s', JSON.stringify(e));
        ...
        _this.daemon.cmd(rpcCommand,
            ...
            function (results) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    emitInfoLog("Response from submitblock: %s", JSON.stringify(result));
                    if (result.error) {
                        emitInfoLog('rpc error with daemon instance %s when submitting block with %s, err = %s', result.instance.index, rpcCommand, result.error);
                        return;
                    }
                    else if (result.response === 'rejected') {
                        emitErrorLog('Daemon instance %s rejected a supposedly valid block', result.instance.index);
                        return;
                    }
                }
                emitWarningLog('Submitted Block using %s successfully to daemon instance(s)', rpcCommand);
                callback();
            }
        );
    ...
    function SetupRecipients() {
        ...
        for (var r in options.rewardRecipients) {
            ...
            try {
                ...
                emitErrorLog('Error generating transaction output script for %s in rewardRecipients', r);
            }
        }
    ...
    function SetupDaemonInterface(finishedCallback) {
        ...
        _this.daemon.once('online', function () {
            ...
            emitErrorLog('Failed to connect daemon(s): %s', JSON.stringify(error));
    ...
    function DetectCoinData(finishedCallback) {
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            if (error || !results) {
                emitErrorLog('Could not start pool, error with init batch RPC call: %s', JSON.stringify(error));
                return;
            }
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (rpcCall !== 'submitblock' && (r.error || !r.result)) {
                    emitErrorLog('Could not start pool, error with init RPC %s - %s', rpcCall, JSON.stringify(r.error));
                    return;
                }
            }
		...
		options.testnet = (rpcResults.getinfo.chain === 'test') ? true : false;
            ...
            if (rpcResults.submitblock.message === 'Method not found') {
                ...
                emitErrorLog('Could not detect block submission RPC method, %s', JSON.stringify(results));
                return;
            }
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            emitInfoLog('No new blocks for %s seconds - updating transactions & rebroadcasting work', options.jobRebroadcastTimeout);
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                emitInfoLog('mining.submit event, params = %s', JSON.stringify(params));
...
//                emitInfoLog('result from jobManager process share = %s', JSON.stringify(result));
                resultCallback(result.error, result.result ? true : null);

            }).on('malformedMessage', function (message) {
                emitWarningLog('Malformed message from %s : %s', client.getLabel(), message);

            }).on('socketError', function (err) {
                emitWarningLog('Socket error from %s : %s', client.getLabel(), JSON.stringify(err));

            }).on('socketTimeout', function (reason) {
                emitInfoLog('Connected timed out for %s : %s',client.getLabel(), reason)

            }).on('socketDisconnect', function () {
                emitInfoLog('Socket disconnected from ' + client.getLabel());

            }).on('kickedBannedIP', function (remainingBanTime) {
                //todo to string interpolation, i'm tired
                emitInfoLog('Rejected incoming connection from ' + client.remoteAddress + ' banned for ' + remainingBanTime + ' more seconds');

            }).on('forgaveBannedIP', function () {
                //todo to string interpolation, i'm tired
                emitInfoLog('Forgave banned IP ' + client.remoteAddress);
    ...
    function SetupBlockPolling() {
        if (typeof options.blockRefreshInterval !== "number" || options.blockRefreshInterval <= 0) {
            emitInfoLog('Block template polling has been disabled');
            return;
        }
...
//                    emitInfoLog('Pool' + coin + 'Thread ' + (parseInt(forkId) + 1) + 'Block notification via RPC polling');
                }
            });
        }, pollingInterval);
    }
    ...
    this.processBlockNotify = function (blockHash, sourceTrigger) {
        emitInfoLog('Block notification via p2p', sourceTrigger);
        if (typeof(_this.jobManager.currentJob) !== 'undefined' && blockHash !== _this.jobManager.currentJob.rpcData.previousblockhash) {
            GetBlockTemplate(function (error, result) {
                if (error) {
                    emitErrorLog('Block notify error getting block template for %s', options.coin.name);
                }
            })
        }
    };
