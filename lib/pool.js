...
//d3bug                emitLog('No new blocks for ' + options.jobRebroadcastTimeout + ' seconds - updating transactions & rebroadcasting work');
...
//          if (typeof options.blockRefreshInterval !== "number" || options.blockRefreshInterval <= 0) {
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(function (error, result, foundNewBlock) {
                if (foundNewBlock) {
//d3bug                    emitLog('Block notification via RPC polling');
                }
            });
        }, pollingInterval);
    }
 ...
 //d3bug       emitInfoLog('Block notification via ' + sourceTrigger);
