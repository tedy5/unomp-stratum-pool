...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, poolOptions) {
    ...
    if(rpcData.founderreward) {
        var founderRewardSatoshis = rpcData.founderreward.amount;
        var founderRewardAddress = rpcData.founderreward.founderpayee;

        reward -= founderRewardSatoshis;
        rewardToPool -= founderRewardSatoshis;

        var payeeScript = util.addressToScript(founderRewardAddress);
        txOutputBuffers.push(Buffer.concat([
...
//        util.packUInt32LE(rpcData.curtime) : Buffer.from([]);
    ...
    var txComment = txMessages === true ?
        util.serializeString('private-pool') :
//        Buffer.from([]);
        new Buffer([]);
